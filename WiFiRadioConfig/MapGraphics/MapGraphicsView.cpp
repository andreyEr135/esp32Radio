...
void MapGraphicsView::handleChildMouseDoubleClick(QMouseEvent *event)
{
    QPointF mousePoint = _childView->mapToScene(_childView->mapFromGlobal(QCursor::pos()));
    QRectF sceneRect = _childScene->sceneRect();
    const float xRatio = mousePoint.x() - 633347; // sceneRect.width();
    const float yRatio = mousePoint.y() - 327465;// sceneRect.height();
    //qDebug() << xRatio <<  yRatio << this->mapToScene(QPoint(this->width(),this->height()));
    //qDebug() << QPoint(this->width()/2,this->height()/2);
    QPointF GeoPos =  this->mapToScene(QPoint(this->width(),this->height()));
    emit dClicked(GeoPos);
    //QPointF centerGeoPos = this->mapToScene(QPoint(this->width()/2,this->height()/2));
